...
exports.getAvailableGrades = getAvailableGrades;
exports.getGameSettings = getGameSettings;
const index_1 = require("../../../../src/data/preschool/index");
const index_2 = require("../../../../src/data/elementary/index");
const index_3 = require("../../../../src/data/diffEq/index");
const index_4 = require("../../../../src/data/english/index");
const index_5 = require("../../../../src/data/generalKnowledge/index");
/* ── grade key 매핑 ── */
const ENG_CATEGORY_MAP = {
...
function getGradeKey(category) {
    if (category.startsWith("5세"))
        return "preschool-5";
    if (category.startsWith("6세"))
        return "preschool-6";
    if (category.startsWith("7세"))
        return "preschool-7";
    const m = category.match(/^(\d)학년/);
    if (m)
        return `elementary-${m[1]}`;
    if (ENG_CATEGORY_MAP[category])
        return ENG_CATEGORY_MAP[category];
    if (GK_CATEGORY_MAP[category])
        return GK_CATEGORY_MAP[category];
    if (DIFF_EQ_CATEGORY_MAP[category])
        return DIFF_EQ_CATEGORY_MAP[category];
    return "unknown";
}
/* ── 레슨 → 퀴즈 문제 변환 ── */
function extractQuestions(lessons) {
    const out = [];
    for (const lesson of lessons) {
        ...
        for (const p of lesson.problems ?? []) {
            if (p.type !== "choice" || !Array.isArray(p.choices) || p.choices.length < 2)
                continue;
            out.push({
                id: p.id,
                gradeKey,
...
const allQuestions = [
    ...extractQuestions(index_1.preschoolLessons),
    ...extractQuestions(index_2.elementaryLessons),
    ...extractQuestions(index_3.diffEqLessons),
    ...extractQuestions(index_4.englishLessons),
    ...extractQuestions(index_5.generalKnowledgeLessons),
];
/* ── gradeKey 별 인덱스 ── */
const questionsByGrade = new Map();
for (const q of allQuestions) {
    let arr = questionsByGrade.get(q.gradeKey);
    if (!arr) {
        arr = [];
        questionsByGrade.set(q.gradeKey, arr);
    }
    arr.push(q);
}
/* ── 공개 API ── */
function getRandomQuestions(count, gradeKey) {
...
function getAvailableGrades() {
    ...
    for (const [key, label] of Object.entries(GRADE_LABELS)) {
        ...
        if (count > 0)
            grades.push({ key, label, questionCount: count });
    }
    return grades;
}
const SETTINGS = {
...
function getGameSettings(gradeKey) {
